...
class EDA:
    ...
    def vehicle_types():
        ...
        vehicle_types = df['VehicleTypes'].apply(lambda x: [item['Name'] for item in x] if x else [])
        flat_vehicle_types = [item for sublist in vehicle_types for item in sublist]
        plt.figure(figsize=(10, 6))
        plt.pie(pd.Series(flat_vehicle_types).value_counts(), labels=pd.Series(flat_vehicle_types).value_counts().index, autopct='%1.1f%%')
        plt.title('Distribution of Vehicle Types')
        plt.show()

    def mfrID():
        # Distribution of Mfr_ID
